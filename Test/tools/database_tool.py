...
class UniversalDepositInput(BaseModel):
    ...
    pin_number: str
    mobile_number: str
    kyc_details_1: str = Field(
        ..., description="Primary KYC Document: TYPE-VALUE (e.g. SSN-123456789)"
    )
    kyc_details_2: str = Field(
        ..., description="Secondary KYC Document: TYPE-VALUE (e.g. PASSPORT-GH12345)"
    )
    ...
    country_code: str = Field(
        default="IN",
        description="ISO 3166-1 alpha-2 country code (e.g. IN, US, GB, CA, SG).",
    )
    risk_score: Optional[int] = Field(
        default=None,
        description="Risk score from 0-100 for risk assessment purposes only. Does NOT affect fd_status.",
    )
    # Note: fd_status is always set to 'ACTIVE' for newly created fixed deposits.
    # The approval workflow status (approved/needs_approval/rejected) should be tracked
    # separately in a different system or field if needed.
    ...
    def _run(self, query: str) -> str:
        try:
            ...
            if len(result) > 3000:
                result = result[:3000] + "\n... [truncated]"
            return (
                result if result else "Query executed successfully (no rows returned)."
            )
        except Exception as e:
            return f"SQL error: {e}"
    ...
    def _run(
        ...
        try:
            ...
            if not email or not email.strip():
                return "Error: Email field is required and cannot be empty."
            ...
            # Note: fd_status is always set to 'ACTIVE' for newly created fixed deposits.
            # The risk_score is stored for reference but does not determine fd_status.
            ...
            with sqlite3.connect(DB_PATH) as conn:
                ...
                if not _db_schema_migrated:
                    ...
                    if "pan_number" in columns and "kyc_details_1" not in columns:
                        try:
                            cursor.execute(
                                "ALTER TABLE kyc_verification ADD COLUMN kyc_details_1 TEXT"
                            )
                            cursor.execute(
                                "ALTER TABLE kyc_verification ADD COLUMN kyc_details_2 TEXT"
                            )
                            cursor.execute(
                                "UPDATE kyc_verification SET kyc_details_1 = 'PAN-' || pan_number WHERE pan_number IS NOT NULL"
                            )
                            cursor.execute(
                                "UPDATE kyc_verification SET kyc_details_2 = 'AADHAAR-' || aadhaar_number WHERE aadhaar_number IS NOT NULL"
                            )
                    ...
                    for stmt in [
                        ...
                        "ALTER TABLE fixed_deposit ADD COLUMN risk_score INTEGER",
                        "ALTER TABLE fixed_deposit ADD COLUMN fd_status TEXT DEFAULT 'ACTIVE'",
                    ]:
                        try:
                            cursor.execute(stmt)
                        except sqlite3.OperationalError:
                ...
                maturity = (
                    datetime.now() + timedelta(days=30 * tenure_months)
                ).strftime("%Y-%m-%d")

                # For installment-based products, store amount as monthly_installment
                _installment_products = {"RD", "MF", "NPS", "PPF", "SSY"}
                monthly_installment = (
                    initial_amount if product_type in _installment_products else None
                )
                ...
                cursor.execute(
                    ...
                    product_type, monthly_installment, compounding_freq, risk_score,
                    ...
                    (
                        ...
                        interest_rate,
                        maturity,
                        "ACTIVE",  # Always set to ACTIVE for newly created FDs
                        product_type,
                        monthly_installment,
                        compounding_freq,
                        risk_score,
                ...
                """,
                    (
                        ...
                        currency_code,
                        ref_no,
                        f"Opening {product_type} — {bank_name}",
                    ),
                )
                conn.commit()

                status_info = "\nStatus: ACTIVE"  # All newly created FDs start as ACTIVE
                risk_info = (
                ...
                return (
                    ...
                    f"Maturity: {maturity}\nTransaction Ref: {ref_no}{status_info}{risk_info}"
                )
        except Exception as e:
            return f"Error: {e}"
...
class SafeNL2SQLTool(BaseTool):
    ...
    description: str = (
        ...
        "Supports all product types in the product_type column: "
        + ", ".join(DEPOSIT_PRODUCT_TYPES)
        + ". "
...
def get_sql_toolkit_tools(llm=None) -> List:
    ...
    for lc_tool in toolkit.get_tools():
        wrapper = LangChainToolWrapper(
            name=lc_tool.name, description=lc_tool.description
        )
