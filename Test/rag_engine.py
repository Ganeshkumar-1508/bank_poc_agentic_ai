...
# Chunking parameters
CHUNK_SIZE = 1500       # characters per chunk (larger = more complete policy rules per chunk)
...
_TABLE_BLOCK_RE = re.compile(r'(\[TABLE \d+\]\n(?:.*\n)*?)(?=\[TABLE \d+\]|\Z)', re.DOTALL)
        ...
        if end < len(text):
            ...
            if table_region:
                ...
                if paragraph_break > start + chunk_size * 0.3:
                    ...
                    if sentence_break > start + chunk_size * 0.3:
                        end = sentence_break + 2
                    else:
                        line_break = text.rfind("\n", start, end)
                        if line_break > start + chunk_size * 0.3:
                            end = line_break + 1
...
def ingest_document(file_path: Path, category: str = "general",
    ...
    if existing and existing["ids"]:
        return {"file_name": file_path.name, "file_hash": file_hash,
                "total_chunks": len(existing["ids"]), "category": category,
    ...
    try:
        ...
        return {"file_name": file_path.name, "file_hash": file_hash,
                "total_chunks": 0, "category": category, "status": f"error: {str(e)}"}

    if not text or len(text.strip()) < 20:
        return {"file_name": file_path.name, "file_hash": file_hash,
                "total_chunks": 0, "category": category,
                "status": "error: document has too little text content"}

    chunks = chunk_text(text)
    if not chunks:
        return {"file_name": file_path.name, "file_hash": file_hash,
                "total_chunks": 0, "category": category, "status": "error: no chunks created"}
    ...
    for i, chunk in enumerate(chunks):
        ...
        metadatas.append({
            ...
            "category": category, "tags": ",".join(tags) if tags else "",
    ...
    for start_idx in range(0, len(ids), batch_size):
        collection.add(ids=ids[start_idx:start_idx+batch_size],
                       documents=documents[start_idx:start_idx+batch_size],
                       metadatas=metadatas[start_idx:start_idx+batch_size])

    return {"file_name": file_path.name, "file_hash": file_hash,
            "total_chunks": len(chunks), "category": category, "status": "success"}


def ingest_from_bytes(file_bytes: bytes, file_name: str,
    ...
    if ext not in SUPPORTED_EXTENSIONS:
        return {"file_name": file_name, "file_hash": "", "total_chunks": 0,
                "category": category,
...
def retrieve(query: str, n_results: int = 5,
    ...
    try:
        ...
        return [{"content": f"RAG retrieval error: {str(e)}", "file_name": "",
                 "category": "", "chunk_index": -1, "relevance_score": 0, "distance": 1.0}]
    ...
    if results and results["documents"] and results["documents"][0]:
        for i, doc in enumerate(results["documents"][0]):
            ...
            formatted.append({"content": doc, "file_name": metadata.get("file_name", "unknown"),
                              "category": metadata.get("category", ""), "tags": metadata.get("tags", ""),
...
def retrieve_as_text(query: str, n_results: int = 5,
...
def list_documents() -> List[Dict]:
    ...
    for metadata in all_data["metadatas"]:
        ...
        if fh and fh not in docs:
            docs[fh] = {"file_name": metadata.get("file_name", "unknown"), "file_hash": fh,
                        "category": metadata.get("category", ""), "tags": metadata.get("tags", ""),
...
def get_stats() -> Dict:
    collection = _get_collection()
    docs = list_documents()
    return {"total_chunks": collection.count(), "total_documents": len(docs),
            "categories": list(set(d["category"] for d in docs if d.get("category"))),
            "storage_dir": str(CHROMA_DIR), "upload_dir": str(UPLOAD_DIR)}
