...
def create_loan_creation_tasks(agents, borrower_context: str = ""):
    ...
    decision_task = Task(
        description=(
            ...
            "EVALUATION WORKFLOW — follow ALL steps in order:\n\n"
            "STEP 0 — DETERMINE LOAN TYPE (mandatory first step):\n"
            "Before searching any policy documents, determine the borrower's loan type\n"
            "based on their 'purpose' field:\n"
            "  - purpose='debt_consolidation' → DEBT CONSOLIDATION LOAN (unsecured personal loan)\n"
            "  - purpose='home_improvement' → HOME IMPROVEMENT LOAN (unsecured personal loan)\n"
            "  - purpose='credit_card' → CREDIT CARD REFINANCE (unsecured personal loan)\n"
            "  - purpose='medical' or 'major_purchase' or 'wedding' or 'moving' or 'vacation' → PERSONAL LOAN\n"
            "  - purpose='small_business' → SMALL BUSINESS LOAN\n"
            "  - purpose='house' → MORTGAGE LOAN\n"
            "  - purpose='car' → AUTO LOAN\n"
            "  - All other purposes → Unsecured PERSONAL LOAN\n"
            "In your response text, state DETERMINED_LOAN_TYPE: [type] before using any tools. Do NOT call this as a tool or Action — simply type it as plain text in your response.\n"
            "CRITICAL: Only evaluate against thresholds for THIS loan type.\n"
            "Do NOT apply mortgage rules to a personal loan applicant.\n"
            "Do NOT apply secured loan rules to an unsecured loan applicant.\n\n"
            "STEP 1 — Check policy database:\n"
            "Call 'RAG Policy Stats' to verify documents are loaded.\n\n"
            "STEP 2 — Search policy documents:\n"
            "Call 'RAG Policy Search' for each of these 6 categories (one call per category):\n"
            "  - 'credit_score': FICO score requirements and DTI thresholds\n"
            "  - 'risk_assessment': Risk grade definitions and default probability bands\n"
            "  - 'eligibility': Income requirements, employment rules, auto-decline criteria\n"
            "  - 'loan_policy': Loan approval criteria and interest rate tiers\n"
            "  - 'compliance': Verification requirements and fair lending rules\n"
            "  - 'regulatory': TILA, ECOA, FCRA, AML requirements\n\n"
            "STEP 3 — Evaluate borrower:\n"
            "Compare the borrower's metrics against ONLY the rules that apply to their DETERMINED_LOAN_TYPE.\n\n"
            "STEP 4 — Hard decline check:\n"
            "Scan ALL retrieved documents for explicit DECLINE/REJECT/AUTO-DECLINE rules.\n"
            "Compare the borrower's actual values against each hard decline rule.\n"
            "If ANY hard decline rule is violated → decision MUST be REJECTED.\n"
            "Cite the EXACT document name, section, and threshold violated.\n\n"
            "STEP 5 — If no hard decline triggered:\n"
            "Use the ML model as advisory: Grade A/B → LOAN_APPROVED, C/D/E → NEEDS_VERIFY.\n\n"
            "STEP 6 — PRE-OUTPUT VERIFICATION (mandatory before writing JSON):\n"
            "Before outputting your final JSON, verify EVERY claimed violation:\n"
            "  a) The exact threshold number appears VERBATIM in the RAG tool output text.\n"
            "  b) The math is correct: if you say 'X is below Y', verify X < Y numerically.\n"
            "     Example: '5 years is below 3 years' is WRONG — 5 > 3.\n"
            "  c) The rule applies to the CORRECT loan type (not a different product).\n"
            "  d) If you find ANY error, correct it before outputting.\n\n"
            "RULES:\n"
            "- NEVER invent thresholds not in the RAG output. Only cite numbers appearing VERBATIM.\n"
            "- For employment length: use 'emp_length_display' field (e.g., '5 years'), not raw number.\n"
            "- Do NOT fabricate policy references. Only cite documents the tool returns.\n"
            "- The 'conditions' array must contain ACTIONABLE steps (e.g., 'Submit W-2 forms').\n"
            "- The 'policy_references' array must include EVERY document searched (one per category).\n"
            "- If a borrower's metric does NOT violate any threshold, do NOT claim it does.\n\n"
            "OUTPUT FORMAT (strict JSON — no markdown fences):\n"
            "{\n"
            '  "loan_decision": "LOAN_APPROVED" | "NEEDS_VERIFY" | "REJECTED",\n'
            '  "rationale": "Detailed explanation citing specific policy thresholds and document names",\n'
            '  "conditions": ["Actionable requirement 1", "Actionable requirement 2"],\n'
            '  "next_steps": ["Specific step 1", "Specific step 2"],\n'
            '  "policy_references": ["Document Name - Section: threshold cited"]\n'
            "}\n\n"
            "FALLBACK (when no policy documents found):\n"
        ...
        expected_output="JSON with loan_decision, rationale, conditions, next_steps, policy_references",
    ...
    summary_task = Task(
        description=(
            ...
            "- Do NOT mention any specific percentage threshold for DTI, LTV, or any ratio "
            "(e.g., 'DTI of 20%', 'recommended LTV of 80%') unless the loan decision rationale "
            "from context explicitly cites that exact number from a policy document.\n"
            "- Do NOT mention any specific FICO score threshold (e.g., 'minimum FICO of 640', "
            "'target FICO of 700') unless the decision rationale from context explicitly states it.\n"
            "- Do NOT say a metric is 'slightly above' or 'borderline' unless the decision "
            "agent's rationale uses those exact words.\n"
            "- If you don't know the exact policy threshold, say 'below the bank's "
            "required threshold' or 'does not meet the bank's requirements' — do NOT guess a number.\n"
            "- Only describe the borrower's actual data points (their FICO, their DTI, their grade) "
            "and the decision outcome. Do not invent comparison benchmarks."
        ),
        expected_output=(
