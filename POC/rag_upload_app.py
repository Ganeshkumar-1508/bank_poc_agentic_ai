...
st.markdown(
    '<div class="upload-header">📄 RAG Policy Document Manager</div>',
    unsafe_allow_html=True,
)
st.caption(
...
tab1, tab2, tab3 = st.tabs(
    ["📤 Upload Documents", "📚 Document Library", "🔍 Test Search"]
)

# Tab 1: Upload
with tab1:
    st.markdown(
        '<div class="section-header">📤 Upload Policy Documents</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        """
    ...
    if uploaded_files and st.button(
        ...
        for i, file in enumerate(uploaded_files):
            ...
            progress_bar.progress(
                i / len(uploaded_files), text=f"Ingesting: {file.name}..."
            )
            result = ingest_from_bytes(
                file_bytes=file_bytes, file_name=file.name, category=category, tags=tags
            )
        ...
        for result in results:
            if result["status"].startswith("success"):
                st.markdown(
                    f'<div class="success-box">✅ <strong>{result["file_name"]}</strong> — {result["total_chunks"]} chunks ingested</div>',
                    unsafe_allow_html=True,
                )
            elif result["status"].startswith("skipped"):
                st.markdown(
                    f'<div class="info-box">⏭️ <strong>{result["file_name"]}</strong> — {result["status"]}</div>',
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f'<div class="error-box">❌ <strong>{result["file_name"]}</strong> — {result["status"]}</div>',
                    unsafe_allow_html=True,
                )

        st.markdown(
            f"**Summary:** {success_count} ingested, {skipped_count} skipped, {error_count} errors"
        )
        st.rerun()
...
with tab2:
    st.markdown(
        '<div class="section-header">📚 Document Library</div>', unsafe_allow_html=True
    )
    docs = list_documents()
    if not docs:
        st.info(
            "No documents in the RAG database yet. Go to the Upload tab to add policy documents."
        )
    else:
        for doc in docs:
            ...
            with col1:
                ...
                st.caption(
                    f"Category: `{doc['category']}` | Chunks: {doc['total_chunks']} | "
            ...
            with col3:
                if st.button(
                    "🗑️ Delete", key=f"del_{doc['file_hash']}", type="secondary"
                ):
...
with tab3:
    st.markdown(
        '<div class="section-header">🔍 Test Policy Search</div>',
        unsafe_allow_html=True,
    )
    st.markdown(
        "Test the RAG retrieval to verify that your policy documents are indexed correctly."
    )
    ...
    test_n_results = st.slider(
        "Number of Results", min_value=1, max_value=10, value=5, step=1
    )
    ...
    if st.button("🔎 Search", type="primary", use_container_width=True):
        if not test_query.strip():
            ...
            with st.spinner("Searching policy documents..."):
                category_filter = (
                    None if test_category == "All Categories" else test_category
                )
                formatted_text = retrieve_as_text(
                    query=test_query, n_results=test_n_results, category=category_filter
                )
            st.markdown("---")
            if "No relevant policy documents found" in formatted_text:
                ...
                st.text_area(
                    "Formatted Results (as agents would see them)",
                    value=formatted_text,
                    height=500,
                    disabled=True,
                )
                individual_results = retrieve(
                    query=test_query, n_results=test_n_results, category=category_filter
                )
                if individual_results:
                    st.markdown("---")
                    st.markdown(
                        '<div class="section-header">📊 Result Details</div>',
                        unsafe_allow_html=True,
                    )
                    for i, result in enumerate(individual_results, 1):
                        with st.expander(
                            f"Result {i}: {result['file_name']} (Relevance: {result['relevance_score']:.1%})",
                            expanded=(i <= 2),
                        ):
                            st.markdown(f"**Source:** {result['file_name']}")
                            st.markdown(f"**Category:** {result['category']}")
                            st.markdown(
                                f"**Relevance Score:** {result['relevance_score']:.1%}"
                            )
                            st.markdown("---")
