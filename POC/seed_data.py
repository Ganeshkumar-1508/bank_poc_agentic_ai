...
ACCOUNT_TYPES = ["SAVINGS", "SAVINGS", "SAVINGS", "CURRENT", "SAVINGS",
                 "SAVINGS", "CURRENT", "SAVINGS", "SALARY",  "SAVINGS"]
...
def seed(conn: sqlite3.Connection):
    ...
    fd_ids = []
    for dep in DEPOSITS:
        (u_idx, product_type, initial_amount, monthly_installment,
         bank_name, tenure_months, interest_rate, compounding_freq, fd_status) = dep
        uid = user_ids[u_idx]
        maturity = future_date(tenure_months) if fd_status == "ACTIVE" else past_date(random.randint(10, 300))
        cur.execute(
            ...
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 1.0, ?, ?, ?)",
            (uid, product_type, initial_amount, monthly_installment, bank_name,
             tenure_months, interest_rate, compounding_freq, maturity,
             fd_status,
    ...
    for dep_idx, (dep, fd_id) in enumerate(zip(DEPOSITS, fd_ids)):
        ...
        cur.execute(
            ...
            (fd_id, acc_id, uid, amount, CURRENCIES[u_idx], rand_ref(),
             f"Initial {product_type} booking", past_date(random.randint(30, 365)))
        )
        txn_count += 1
        ...
        if fd_status not in ("PREMATURE",):
            for _ in range(random.randint(1, 3)):
                ...
                cur.execute(
                    ...
                    (fd_id, acc_id, uid, interest, CURRENCIES[u_idx], rand_ref(),
                     "Quarterly interest credit", past_date(random.randint(5, 25)))
                )
                txn_count += 1
        ...
        if fd_status == "PREMATURE":
            ...
            cur.execute(
                ...
                (fd_id, acc_id, uid, penalty, CURRENCIES[u_idx], rand_ref(),
                 "Premature withdrawal penalty deducted", past_date(5))
            )
            txn_count += 1
        ...
        if fd_status in ("CLOSED", "MATURED"):
            cur.execute(
                ...
                (fd_id, acc_id, uid, amount, CURRENCIES[u_idx], rand_ref(),
                 f"{fd_status} payout", past_date(random.randint(1, 15)))
            )
            txn_count += 1
    ...
    case_ids = []
    for ac in AML_CASES:
        u_idx, risk_score, risk_band, decision, sanctions_hit, pep_flag, adverse_media = ac
        uid = user_ids[u_idx]
        screened_at = past_date(random.randint(5, 60))
        report_name = f"aml_report_{USERS[u_idx][0].lower()}_{USERS[u_idx][1].lower()}.pdf"
        cur.execute(
            ...
            (uid, risk_score, risk_band, decision,
             f"/reports/{report_name}",
             sanctions_hit, pep_flag, adverse_media,
    ...
    events = [
        ...
        ("AML_CHECK",        "AML screening initiated by onboarding crew."),
...
def print_summary(conn: sqlite3.Connection):
    tables = [
        "users", "address", "kyc_verification", "accounts",
        "fixed_deposit", "transactions", "aml_cases",
        "compliance_audit_log", "interest_rates_catalog",
    ]
    print("\n" + "─" * 45)
...
if __name__ == "__main__":
    ...
    with sqlite3.connect(DB_PATH) as conn:
        ...
        if existing > 0:
            print(f"[WARN] Database already contains {existing} user(s). Skipping seed to avoid duplicates.")
            print("       Delete 'bank_poc.db' and re-run create_db.py + seed_data.py to start fresh.")
            print_summary(conn)
            raise SystemExit(0)
